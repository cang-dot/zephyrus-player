...
- **雨夜播放器样式（Rain）**：全新沉浸式播放器样式，雨夜氛围感
  - Canvas 雨水效果：斜向雨滴粒子，支持音频响应驱动雨水密度和下落速度
  - 落地水洼效果：伪3D反射水洼，雨滴落地产生涟漪
  - 地面涟漪：整个地面都有雨滴涟漪效果，水洼处效果更强
  - 左右分词歌词：将歌词按词分割，左右两列显示，支持逐字高亮
  - 两句一组模式：歌词每两句分一组，当前歌词高亮显示
  - 可配置封面显示：支持显示/隐藏封面，隐藏时歌词合并，中间竖光晕分隔
  - 背景暗化调节：支持 0-100% 背景暗化程度调节
- **移动端适配**：Android 平台支持，使用 Capacitor 框架
- **封面水波效果**：鼠标划过时产生真实水波涟漪效果，使用波动方程模拟物理水波

### 🎨 优化

- **播放器样式系统**：重构播放器样式注册系统，支持动态注册新样式
- **设置面板**：支持动态加载样式设置选项
- **图标系统**：统一更新应用图标
