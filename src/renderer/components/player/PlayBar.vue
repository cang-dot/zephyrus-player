        ...
        color: musicFullVisible
          ? forcedBarTextColor === 'black'
            ...
            : forcedBarTextColor === 'white'
              ? '#ffffff'
              : textColors.theme === 'dark' ? '#000000' : '#ffffff'
          : settingsStore.theme === 'dark' ? '#ffffff' : '#000000'
      }"
    >
      <!-- Progress bar at top -->
      ...
      <div class="audio-button">
        ...
        <div class="audio-volume custom-slider" @wheel.prevent="handleVolumeWheel">
          <div class="volume-icon" @click="mute"><i class="iconfont" :class="getVolumeIcon"></i></div>
          <div class="volume-slider">
            <div class="volume-percentage" :class="{ 'volume-percentage-disabled': isMuted }">{{ Math.round(volumeSlider) }}%</div>
            <n-slider v-model:value="volumeSlider" :step="0.01" :tooltip="false" :disabled="isMuted" vertical></n-slider>
          </div>
        </div>
        ...
        <n-tooltip v-if="!isMobile" trigger="hover" :z-index="9999999">
          <template #trigger>
            <i class="iconfont" :class="[playModeIcon, { 'intelligence-active': playMode === 3 }]" @click="togglePlayMode"></i>
          </template>
          {{ playModeText }}
        </n-tooltip>
        ...
        <n-tooltip v-if="!isMobile" trigger="hover" :z-index="9999999">
          <template #trigger>
            <i class="iconfont" :class="{ 'like-active': isFavorite, 'ri-heart-3-fill': isFavorite, 'ri-heart-3-line': !isFavorite }" @click="toggleFavorite"></i>
          </template>
        ...
        <n-tooltip v-if="isElectron" trigger="hover" :z-index="9999999">
          <template #trigger>
            <i class="iconfont ri-netease-cloud-music-line" :class="{ 'text-[var(--accent-color)]': isLyricWindowOpen, 'disabled-icon': !playMusic?.id }" @click="playMusic?.id && openLyricWindow()"></i>
          </template>
        ...
        <n-tooltip trigger="hover" :z-index="9999999" v-if="isElectron">
          <template #trigger>
            <i class="iconfont ri-soundcloud-fill" :class="{ 'disabled-icon': !playMusic?.id }" @click="playMusic?.id && (showClimaxEditor = true)"></i>
          </template>
          标记高潮
        </n-tooltip>
        ...
        <n-tooltip v-if="metaphorButtonVisible" trigger="hover" :z-index="9999999">
          <template #trigger>
            <i class="iconfont ri-quill-pen-line" :class="{ 'text-[var(--accent-color)]': showMetaphorPanel }" @click="showMetaphorPanel = !showMetaphorPanel"></i>
          </template>
          歌词隐喻分析
        </n-tooltip>
          ...
          <div class="mini-slots">
            <template v-for="(slot, i) in miniSlots" :key="i">
              <div class="mini-slot">
                ...
                <!-- Favorite -->
                <div v-else-if="slot === 'favorite'" class="bar-btn" :class="{ 'like-active': isFavorite }" @click="toggleFavorite">
                  <i class="iconfont" :class="isFavorite ? 'ri-heart-3-fill' : 'ri-heart-3-line'"></i>
                </div>
                <!-- Lyric -->
                <div v-else-if="slot === 'lyric'" class="bar-btn" :class="{ 'disabled-icon': !playMusic?.id }" @click="playMusic?.id && openLyricWindow()">
                  <i class="iconfont ri-netease-cloud-music-line" :class="{ 'text-[var(--accent-color)]': isLyricWindowOpen }"></i>
                </div>
                <!-- Playlist -->
                ...
                <!-- Volume -->
                <div v-else-if="slot === 'volume'" class="bar-action-volume" @wheel.prevent="handleVolumeWheel">
                  <div class="volume-inner">
                    <div class="volume-slider"><n-slider v-model:value="volumeSlider" :step="0.01" :tooltip="false" :disabled="isMuted"></n-slider></div>
                    <div class="bar-btn volume-icon" @click="mute"><i class="iconfont" :class="getVolumeIcon"></i></div>
                  </div>
                </div>
                <!-- Advanced (EQ + timer + speed) -->
                <advanced-controls-popover v-else-if="slot === 'advanced'" />
                <!-- Climax -->
                <div v-else-if="slot === 'climax'" class="bar-btn" :class="{ 'disabled-icon': !playMusic?.id }" @click="playMusic?.id && (showClimaxEditor = true)">
                  <i class="iconfont ri-soundcloud-fill"></i>
                </div>
                <!-- Metaphor -->
                <div v-else-if="slot === 'metaphor' && metaphorButtonVisible" class="bar-btn" :class="{ 'text-[var(--accent-color)]': showMetaphorPanel }" @click="showMetaphorPanel = !showMetaphorPanel">
...
import {
  allTime, artistList, getLyricTextAtTime, isLyricWindowOpen, lrcArray, nowTime, openLyric, playMusic, textColors
...
const miniSlots = computed(() => settingsStore.setData?.playBarMiniSlots || ['playMode', 'favorite', 'playlist', 'volume']);
...
const startDragMiniBar = (e: MouseEvent) => {
  ...
  const onUp = () => {
    if (miniBarX.value >= 0) {
      try { localStorage.setItem('miniBarPosition', String(miniBarX.value)); } catch {}
    }
...
function resetCollapseTimer() {
  ...
  collapseTimer = setTimeout(() => { playBarCollapsed.value = true; }, COLLAPSE_DELAY);
}
...
watch(musicFullVisible, (visible) => {
  if (visible) {
    try {
      ...
      if (saved) {
        const cfg = JSON.parse(saved);
        if (cfg.playerStyle === 'frenzy') { playBarCollapsed.value = false; return; }
      }
    } catch {}
    resetCollapseTimer();
  } else {
...
.fp-tooltip {
  ...
  .slider-hover-lyric, .fp-tooltip-lyric { font-size: 12px; color: #fff; max-width: 200px; overflow: hidden; text-overflow: ellipsis; margin-bottom: 2px; }
  .slider-hover-time, .fp-tooltip-time { font-size: 10px; color: rgba(255, 255, 255, 0.6); font-family: monospace; }
}

// ==================== Climax segments (shared) ====================
.climax-track, .fp-climax { position: absolute; top: 0; left: 0; right: 0; height: 4px; pointer-events: none; z-index: 1; }
.climax-segment, .fp-climax-seg {
  position: absolute; height: 100%; border-radius: 2px;
  background: rgba(255, 200, 50, 0.35); transition: background 0.2s ease;
  &.climax-active, &.fp-climax-active { background: rgba(255, 200, 50, 0.6); }
}
...
.music-play-bar {
  ...
  .music-content {
    ...
    &-name { @apply text-xs mt-1 opacity-80; }
  }
}
...
.play-bar-img-wrapper {
  ...
  .hover-arrow {
    ...
    .hover-content { @apply flex flex-col items-center justify-center; i { @apply text-white mb-0.5; } .hover-text { @apply text-white text-xs scale-90; } }
  }
  &:hover .hover-arrow { @apply opacity-100; }
}

.play-bar-img { @apply w-14 h-14 rounded-2xl; }

.music-buttons {
  @apply mx-6 flex-1 flex justify-center items-center;
  .iconfont { @apply text-2xl transition hover:text-[var(--accent-color)]; }
  .icon { @apply text-3xl hover:text-[var(--accent-color)]; }
  > div { @apply cursor-pointer; }
  &-play { @apply flex justify-center items-center w-20 h-12 rounded-full mx-4 transition text-gray-500; @apply bg-gray-100 bg-opacity-60 dark:bg-gray-800 dark:bg-opacity-60 hover:bg-gray-200; }
}

.audio-volume {
  @apply flex items-center relative;
  &:hover .volume-slider { @apply opacity-100 visible; }
  .volume-icon { @apply cursor-pointer; }
  .iconfont { @apply text-2xl transition hover:text-[var(--accent-color)]; }
  .volume-slider {
    ...
    .volume-percentage {
      ...
      &.volume-percentage-disabled { @apply text-gray-400 dark:text-gray-500; }
    }
  }
}
...
.audio-button {
  ...
  .iconfont { @apply text-2xl transition cursor-pointer mx-3 hover:text-[var(--accent-color)]; }
}
...
.loading-overlay { @apply absolute inset-0 flex items-center justify-center rounded-2xl; background-color: rgba(0,0,0,0.5); z-index: 2; }
.loading-icon { font-size: 24px; color: white; animation: spin 1s linear infinite; }
...
.custom-slider {
  :deep(.n-slider) {
    ...
    .n-slider-rail { @apply overflow-hidden transition-all duration-200; background-color: rgba(128, 128, 128, 0.12) !important; }
    .n-slider-handle { @apply transition-all duration-200; opacity: 0; }
    &:hover .n-slider-handle { opacity: 1; }
  }
  :deep(.n-slider-handle .n-slider-tooltip) { display: none !important; }
}
...
.floating-bar.mini-bar {
  ...
  transition: width 0.35s cubic-bezier(0.2, 0, 0.1, 1),
...
.mini-bar-handle {
  ...
  .mini-bar:hover & { opacity: 0.6; pointer-events: auto; }
  &:active { cursor: grabbing; opacity: 0.8; }
  :global(.dark) & { background: rgba(255, 255, 255, 0.25); }
}
...
.mini-bar-progress {
  ...
  overflow: visible;
  z-index: 9998;
  transition: width 0.35s cubic-bezier(0.2, 0, 0.1, 1), left 0s;
  ...
  .n-slider .n-slider-rail__fill { background: var(--accent-color, #888) !important; }
  :global(.dark) & .n-slider { --n-rail-color: rgba(255, 255, 255, 0.1) !important; }
}
...
// Song info
.bar-song-info { flex: 1; min-width: 0; margin-left: 12px; overflow: hidden; }
...
.mini-expanded-area {
  ...
  max-width: 0;
  overflow: hidden;
  transition: opacity 0.25s ease, max-width 0.35s cubic-bezier(0.2, 0, 0.1, 1);

  &.area-visible { opacity: 1; max-width: 300px; }
}
...
.bar-btn-play {
  ...
  .iconfont { font-size: 22px; transition: color 0.2s ease; &:hover { color: var(--accent-color, #888); } }
}

// 4-slot grid
.mini-slots { display: flex; align-items: center; gap: 2px; flex-shrink: 0; }
.mini-slot { display: flex; align-items: center; justify-content: center; }

// Volume in mini slot
.bar-action-volume {
  position: relative;
  .volume-inner { display: flex; align-items: center; }
  &:hover .volume-slider { width: 60px; opacity: 1; margin-right: 4px; }
...
// Mobile overrides
// ====================================================================
:global(.mobile) .music-play-bar { @apply px-4 bottom-[56px]; }
:global(.mobile) .music-time { display: none; }
:global(.mobile) .audio-volume { display: none; }
:global(.mobile) .music-buttons { @apply m-0; &-prev, &-next { display: none; } &-play { @apply m-0; } }
:global(.mobile) .music-content { flex: 1; }
</style>
